...
form.addEventListener('submit', e => {
    e.preventDefault();
    try{
        validateFild('#tracking', (el) =>{
            el.error()
            Swl.fire({
                icon: 'error',
                title: 'Favor de Agregar Numero de tracking'
            })
        })
        validateFild('#cost', (el) =>{
            el.error()
            Swl.fire({
                icon: 'error',
                title: 'Favor agregar costo del producto'
            })
        })
        validateFild('#file', (el) =>{
            el.error()
            Swl.fire({
                icon: 'error',
                title: 'Favor agregar factura'
            })
        })
        validateFild('#id_category', (el) =>{
            el.error()
            Swl.fire({
                icon: 'error',
                title: 'Favor selecionar categoria'
            })
        })
        Xhr.send({
            url: 'pre-alert-add',
